...
const List = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try{
        let { data, status } = await axios.get(`http://localhost:4000/api/hotels/city/${destination}?min=${min || 0}&max=${max || 99999}`)
        if (status === 200) 
          setResults(data);
      } catch(err){
        setResults([]);
      ...
      <div className="listContainer">
        <div className="listWrapper">
          <div className="listSearch">
            ...
            <button onClick={()=>setRefetch(!refetch)}>Search</button>
          </div>
          <div className="listResult">
            {
              results.length !=0 &&
              results.map(result=>(
                <SearchItem hotel={result}/>
              ))
              || 
